...
data Path = Leftp Path | Rightp Path | End 
  deriving (Eq, Show)
...
search a (Leaf b) | a == b = (Just End)
                  | otherwise = Nothing
search a (Node left right) = (mapMaybe (\x -> Leftp x) (search a left) )<|> (mapMaybe (\x-> Rightp x) (search a right))
