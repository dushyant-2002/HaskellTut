...
countWhiteSpaceStrings (x:xs) = foldl' (\acc x -> if all isSpace x then (acc+1) else acc) 0 xs
...
------------made count of total whitespaces in all the string present inside list of strings by mistake please review it also---------
-- countWhiteSpaceStrings [] = 0
-- countWhiteSpaceStrings (str : strArr) = foldl' (\acc x -> if x == ' ' then acc + 1 else acc) 0 str + countWhiteSpaceStrings strArr
...
removeLongWords n = unwords . filter (\str -> (length str <= n) ) . words
...
-- MkPos {pUp = 5, pRight = 4}
goDir :: Pos -> Dir -> Pos
goDir (MkPos a b) Lft   = (MkPos a (b-1))
goDir (MkPos a b) Rgt   = (MkPos a (b+1))
goDir (MkPos a b) Up    = (MkPos (a+1) b)
goDir (MkPos a b) Down  = (MkPos (a-1) b)
...
goDirs :: Pos -> [Dir] -> Pos
-- goDirs p  [] = p
-- goDirs p (ds:dirs) = (goDirs ((goDir p ds)) dirs)
goDirs  = foldl' goDir 
...
mapBinTree f    (Bin Empty a Empty)     = (Bin Empty (f a) Empty)
mapBinTree f    (Bin  a b c)            = Bin (mapBinTree f a) (f b) (mapBinTree f c)
...
labelTree tree = fst (labelTreeAux 1 tree)

labelTreeAux :: Int -> BinTree a -> (BinTree (a, Int), Int)
labelTreeAux n Empty                = (Empty, n)
labelTreeAux n (Bin left x right)   =
  (Bin leftcall (x, n') rightcall, n'')
  where
    (leftcall, n')      = labelTreeAux n left
    (rightcall, n'')    = labelTreeAux (n' + 1) right

-- labelTree  = helper 1
--     where 
--         helper _ Empty = Empty
--         helper acc (Bin Empty a Empty) = helper (acc + 1) (Bin Empty (a,acc) Empty)
...
takeAux = foldr (\x f n -> if n <= 0 then [] else x : f (n - 1)) (const [])
